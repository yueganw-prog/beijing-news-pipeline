...
// ---- Generated articles for realistic demo ----
const genArticles: Article[] = [];
const SOURCES = [
  { name: "36氪", category: "tech" as const },
  { name: "虎嗅", category: "tech" as const },
  { name: "IT之家", category: "tech" as const },
  { name: "新浪财经", category: "finance" as const },
  { name: "第一财经", category: "finance" as const },
  { name: "经济观察报", category: "finance" as const },
  { name: "新京报", category: "local" as const },
  { name: "北京日报", category: "local" as const },
  { name: "北京商报", category: "local" as const },
];
const AUTHORS = ["张颖", "李想", "玄隐", "王晓", "陈晨", "刘明", "张强", "李华", "赵磊", "罗辑", "黄渊普"];
const TITLES = {
  tech: ["AI大模型突破", "年度技术趋势发布", "科技投融资回顾", "开源AI框架发布", "量子计算商用化", "创始人谈创新", "自动驾驶新进展", "战略融资完成", "5G-A网络覆盖", "智能硬件发布", "云原生技术应用", "CTO技术路线图"],
  finance: ["上半年财报发布", "汇率创阶段新高", "企业IPO审核提速", "央行逆回购操作", "A股板块领涨", "国际油价震荡", "数字人民币试点", "行业监管新政", "全球供应链重构", "债券市场动态", "私募基金布局", "社保基金增持"],
  local: ["区域城市更新", "示范学校新增", "地铁建设进展", "京津冀项目落地", "公园改造开放", "中关村论坛举办", "商圈消费升温", "副中心配套开工", "老旧小区改造", "国际电影节开幕", "基础设施项目批复", "博物馆评估"],
};
const KWS = {
...
for (let i = 36; i <= 250; i++) {
  const s = SOURCES[i % 9];
  const ts = TITLES[s.category];
  const ks = KWS[s.category];
  const title = ts[i % ts.length] + "·" + ks[i % ks.length];
  const au = AUTHORS[i % AUTHORS.length];
  const hrs = Math.max(1, 120 - i * 0.4);
  genArticles.push({
    id: i,
    source: s.name,
    category: s.category,
    ...
    url: "#",
    author: au,
    summary: `${title}。${au}报道，该动态预计将对${s.category === "tech" ? "科技" : s.category === "finance" ? "财经" : "本地"}行业产生影响。`,
    published_at: ago(Math.floor(hrs), Math.floor((i * 7) % 60)),
    fetched_at: ago(Math.floor(hrs * 0.9), 0),
  });
}

export const mockArticles: Article[] = [...handCrafted, ...genArticles];
